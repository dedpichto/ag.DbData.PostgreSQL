![Nuget](https://img.shields.io/nuget/v/ag.DbData.PostgreSQL)

# ag.DbData.PostgreSQL
...
4. Inject IPostgreSQLDbDataFactory into your classes:
...
- [ag.DbData.PostgreSQL](https://www.nuget.org/packages/ag.DbData.PostgreSQL/)
...
```csharp
IServiceCollection AddAgPostgreSQL(this IServiceCollection services)
```

Appends the registration of ```IDbDataFactory``` and ```IDbDataObject``` to ```IServiceCollection```.

```csharp
IServiceCollection AddAgPostgreSQL(this IServiceCollection services, IConfigurationSection configurationSection)
```
...
```csharp
IServiceCollection AddAgPostgreSQL(this IServiceCollection services, Action<DbDataSettings> configureOptions)
```
...
ag.DbData.PostgreSQL is built with the following projects:
